...
function check() {
    ...
    DIR=${1}
    docker run -d --rm --name fib_remote --network fib_network -v ${PWD}/${DIR}:/code -w /code --entrypoint "/bin/bash" -e OTEL_EXPORTER_OTLP_ENDPOINT="http://fib_collector:4137" -e OTEL_SERVICE_NAME="fibonacci-remote" -e APPLICATION_PORT=8090 -p 8090:8090 node:lts -c "./run.sh"
    docker run -d --rm --name fib_service --network fib_network -v ${PWD}/${DIR}:/code -w /code --entrypoint "/bin/bash" -e OTEL_EXPORTER_OTLP_ENDPOINT="http://fib_collector:4137" -e OTEL_SERVICE_NAME="fibonacci-server" -e LEFT_REMOTE_SERVICE="http://fib_remote:8090" -e RIGHT_REMOTE_SERVICE="http://fib_remote:8090"  -e APPLICATION_PORT=8080 -p 8080:8080 node:lts -c "./run.sh"
    while true; do
...
function generate() {
    ...
    ${UNIFDEF_WITH_VARS} _base/app.js > "${DIR}/app.js"
    ${UNIFDEF_WITH_VARS} _base/lib.js > "${DIR}/lib.js"
    npx prettier -w ${DIR}
    node --check ${DIR}/app.js
    node --check ${DIR}/lib.js
    check ${DIR}
}
